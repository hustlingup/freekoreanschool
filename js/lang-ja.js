...
const LangManager = (() => {
  ...
  function t(key) { return (_lang === 'ja' && JA[key]) ? JA[key] : key; }

  function init() {
    _lang = localStorage.getItem(LS_KEY) || 'en';
    if (_lang === 'ja') _applyJa();
    _renderBtn(_lang);
    _renderHeroPicker();
  }
  ...
  function _applyJa() {
    ...
    document.querySelectorAll('input[placeholder]').forEach(el => {
      if (JA[el.placeholder]) el.placeholder = JA[el.placeholder];
    });
      ...
      for (const m of mutations) {
        ...
        m.addedNodes.forEach(n => {
          if (n.nodeType === Node.ELEMENT_NODE) {
            ...
            if (JA[txt]) n.textContent = JA[txt];
          }
        });
      }
    });
  ...
  function _renderHeroPicker() {
    ...
    picker.innerHTML = `
      <span class="lang-hero-label">Choose your language / 言語を選択してください</span>
  ...
  function _renderBtn(lang) {
    ...
    if (!btn) return;
    btn.textContent = lang === 'ja' ? '🇯🇵 日本語' : '🇺🇸 EN';
  ...
  return { init, setLang, getLang, t, translateNode: _translateNode };
})();
